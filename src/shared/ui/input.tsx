...
export const Input = forwardRef(function Input(
  ...
  const [unitContainerWidth, setUnitContainerWidth] = useState(0);

  useLayoutEffect(() => {
    if (unitContainerRef.current && unit) {
      const width = unitContainerRef.current.getBoundingClientRect().width;
      setUnitContainerWidth(width);
    }
  }, [unit]);
    ...
    >
      <Headless.Input
        ...
        style={
          unit
            ? {
                paddingRight: unitContainerWidth + 4,
              }
            : undefined
        }
      />
      ...
      >
        <span className="text-zinc-500 sm:text-sm">{unit}</span>
      </div>
    </span>
  );
});
